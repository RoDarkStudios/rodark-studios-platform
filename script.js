...
function getVerifyDisplayName(user) {
    if (!user || typeof user !== 'object') {
        return '';
    }

    return String(user.globalName || user.displayName || user.username || '').trim();
}

function verificationMatchesCurrentUsers(status) {
    const verification = status && status.verification;
    const robloxUser = status && status.robloxUser;
    const discordUser = status && status.discordUser;
    if (!verification || !robloxUser || !discordUser) {
        return false;
    }

    return String(verification.robloxUserId) === String(robloxUser.id)
        && String(verification.discordUserId) === String(discordUser.id);
}

async function fetchWebsiteVerifyStatus() {
...
function setVerifyCardState(card, isConnected) {
    if (!card) {
        return;
    }

    card.classList.toggle('is-connected', Boolean(isConnected));
}

function renderWebsiteVerifyStatus(status) {
    const discordCard = document.getElementById('verify-discord-card');
    const robloxCard = document.getElementById('verify-roblox-card');
    const discordStatus = document.getElementById('verify-discord-status');
    const robloxStatus = document.getElementById('verify-roblox-status');
    ...
    const robloxUser = status && status.robloxUser;
    const verification = status && status.verification;
    const isCurrentVerified = verificationMatchesCurrentUsers(status);
    const isKnownVerified = Boolean(verification && status && status.isVerified);

    setVerifyCardState(discordCard, discordUser || isKnownVerified);
    setVerifyCardState(robloxCard, robloxUser || isKnownVerified);

    if (discordUser) {
        discordStatus.textContent = `Connected as ${getVerifyDisplayName(discordUser) || discordUser.username}`;
    } else if (verification) {
        discordStatus.textContent = `Verified Discord ID ${verification.discordUserId}`;
    } else {
        discordStatus.textContent = 'Not connected';
    }

    if (robloxUser) {
        robloxStatus.textContent = `Signed in as @${robloxUser.username}`;
    } else if (verification) {
        robloxStatus.textContent = `Verified Roblox ID ${verification.robloxUserId}`;
    } else {
        robloxStatus.textContent = 'Not signed in';
    }

    if (discordButton) {
        discordButton.hidden = Boolean(discordUser);
    }
    if (robloxButton) {
        robloxButton.hidden = Boolean(robloxUser);
    }
    if (completeButton) {
    ...
    } else if (isKnownVerified && verification) {
        result.textContent = `Already verified: Discord ID ${verification.discordUserId} is linked to @${verification.robloxUsername || verification.robloxUserId}.`;
    } else if (status && status.canComplete) {
        result.textContent = 'Both accounts are connected. Saving verification...';
    } else {
        result.textContent = 'Connect Discord and sign in with Roblox to finish verification.';
    }
}

async function initWebsiteVerifyPage() {
    ...
    async function completeVerification() {
        ...
        if (result) {
            result.textContent = 'Saving verification...';
        }

        try {
            await postJson('/api/verify/complete', {});
            const nextStatus = await fetchWebsiteVerifyStatus();
            renderWebsiteVerifyStatus(nextStatus);
        } catch (error) {
            if (result) {
                result.textContent = error.message || 'Verification failed. Try again.';
            }
        } finally {
            completing = false;
        }
    ...
    async function refreshStatus() {
        try {
            const status = await fetchWebsiteVerifyStatus();
            renderWebsiteVerifyStatus(status);
            if (status.canComplete && !verificationMatchesCurrentUsers(status)) {
                await completeVerification();
            }
        } catch (error) {
            if (result) {
                result.textContent = error.message || 'Failed to load verification status.';
            }
        }
    }
    ...
    if (discordButton) {
        discordButton.addEventListener('click', () => {
            window.location.href = `/api/verify/discord/login?returnTo=${encodeURIComponent('/verify')}`;
        });
    }

    if (robloxButton) {
        robloxButton.addEventListener('click', handleRobloxLogin);
    }
